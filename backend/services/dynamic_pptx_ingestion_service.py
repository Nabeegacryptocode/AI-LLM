"""
Dynamic PPTX Ingestion Service
Automatically downloads and ingests PPTX files found by Discovery Engine
"""
import logging
import tempfile
import os
...
class DynamicPPTXIngestionService:
    """Service for dynamically ingesting PPTX files from GCS"""
    ...
    async def process_discovery_results(
        ...
        """
        Process Discovery Engine results and ingest PPTX files
        ...
        for result in discovery_results:
            ...
            # Download and ingest
            logger.info(f"Processing PPTX: {result.get('title', 'Unknown')}")
            ingest_result = await self.download_and_ingest_pptx(
                url,
                namespace=namespace,
                chunk_by_slide=chunk_by_slide
            )
            ...
            if ingest_result.get('success'):
                ...
                results['files'].append({
                    ...
                    'slides': ingest_result.get('total_slides', 0)
                })
            else:
                results['failed'] += 1
                results['files'].append({
    ...
    async def get_content_from_discovery_results(
        ...
        """
        # First, ingest any PPTX files found
