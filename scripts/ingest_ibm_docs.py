"""
Script to scrape and ingest IBM documentation
"""
import sys
import os
import asyncio
...
async def ingest_ibm_cloud_docs(
    sections: list = None,
    max_pages_per_section: int = 20,
    namespace: str = "ibm-cloud"
):
    """
    Scrape and ingest IBM Cloud documentation
    ...
    """
    try:
        logger.info("Starting IBM Cloud documentation ingestion")
        
        # Determine which sections to scrape
        if sections:
            section_urls = {k: v for k, v in IBM_CLOUD_SECTIONS.items() if k in sections}
        else:
            section_urls = IBM_CLOUD_SECTIONS
...
async def test_search(query: str, namespace: str = "ibm-cloud"):
    """
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description='Ingest IBM documentation')
    parser.add_argument(
        '--sections',
        nargs='+',
        choices=list(IBM_CLOUD_SECTIONS.keys()) + ['all'],
        default=['overview'],
    ...
    parser.add_argument(
        ...
        type=str,
        default='ibm-cloud',
        help='Pinecone namespace (default: ibm-cloud)'
    )
    parser.add_argument(
        '--test-query',
        type=str,
    ...
    # Run ingestion
    result = asyncio.run(ingest_ibm_cloud_docs(
        sections=sections,
